...
import { htmlToText } from "html-to-text"; // Add this import if not already
...
export async function generateMetadata({ params }) {
    ...
    const job = await fetch(`${baseUrl}/api/bullhorn/job/${id}`).then(res => res.json());

    const desc = job.description?.slice(0, 160) || job.title || "Explore job openings at Timpl";
    const url = `${baseUrl}/job/${job.id}`;

    const defaultOgImage = "https://timpl-next-js.vercel.app/og-image.jpg";
    ...
    const jobPostingSchema = {
        ...
        "datePosted": new Date(job?.datePosted || job?.dateModified || new Date()).toISOString(),
    ...
    return {
        ...
        other: {
            "script:ld+json": JSON.stringify(jobPostingSchema)
        }
    };
}
            ...
            <div className="min-h-screen">
                ...
                <div className="container mx-auto py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-8 xl:px-24 bg-white min-h-screen">
                    <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-6 sm:gap-8 lg:gap-12">
                        ...
                        {/* Right Column: Find A Job & Job Information */}
                                ...
                                <form className="space-y-4">
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            className="pl-10 sm:pl-12 pr-4 py-2 sm:py-3 rounded-md border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 w-full text-sm sm:text-base"
                                            aria-label="Email address"
                                        />
                                    </div>
                                    <Button className="w-full bg-[#23baa1] hover:bg-[#23baa1] text-white font-semibold py-2 sm:py-3 rounded-md text-sm sm:text-base">
                                        Get Job Alert
                                    </Button>
                                </form>
                            </div> */}
